...
const knownRoomsCanon = new Set();     // formes canoniques (location exactes)
let filteredItems = [];                // items visibles dans le menu
let activeIndex = -1;                  // index surligné (clavier)
...
async function init() {
  ...
  document.addEventListener('click', (e) => { if (!roomBox.contains(e.target)) closeMenu(); });

  /* === DATE : clic partout ouvre === */
...
/* ================= Menu salle ================= */
function openMenu(){ roomBox.setAttribute('aria-expanded','true'); }
function closeMenu(){ roomBox.setAttribute('aria-expanded','false'); activeIndex = -1; markActive(); }
...
function populateMenu(filter){
  ...
  filteredItems.slice(0, 400).forEach((name, i) => {
    ...
    li.dataset.value = name; li.dataset.index = i;
    li.textContent = name; roomMenu.appendChild(li);
  });
  markActive();
}
...
function buildRoomsIndex(events){
  roomsDisplay = []; knownRoomsCanon.clear();
  const seen = new Set();
  for (const ev of events) {
    const loc = (ev.location || '').trim(); if (!loc) continue;
    const can = canonical(loc); if (can.length < 3 || seen.has(can)) continue;
    seen.add(can); knownRoomsCanon.add(can); roomsDisplay.push(loc);
  }
...
function renderForSelectedDay(){
  const selected = fromInputDate(dayPicker.value); if (!selected) return;
  ...
  listEl.innerHTML = '';
  const head = document.createElement('div'); head.className = 'day';
  head.textContent = cap(fmtDate.format(start)); listEl.appendChild(head);
...
function renderEvent(ev){
  ...
  const title = document.createElement('div'); title.className = 'title'; title.textContent = ev.title || '(Sans titre)';
