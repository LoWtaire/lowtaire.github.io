...
function bindEvents() {
  ...
  ui.campusSelect.addEventListener('change', async () => {
    const campus = state.campusConfig.find((item) => item.id === ui.campusSelect.value);
    if (!campus) return;
    state.selectedCampus = campus;
    state.detection = { quality: 'manual', banner: 'Campus choisi manuellement.', distanceM: null };
...
async function initCampus() {
  ...
  const savedCampus = state.campusConfig.find((item) => item.id === savedId);
  const picked = savedCampus
    ? { campus: savedCampus, quality: 'manual', banner: '', distanceM: null }
    : state.campusConfig.length === 1
      ? { campus: state.campusConfig[0], quality: 'manual', banner: '', distanceM: null }
      : await pickCampusByGeo(state.campusConfig);
  state.selectedCampus = picked.campus || state.campusConfig[0] || null;
  state.detection = picked;
  ui.campusSelect.innerHTML = state.campusConfig
    .map((campus) => `<option value="${escapeAttr(campus.id)}">${escapeHtml(campus.name)}</option>`)
    .join('');
  ui.campusSelect.closest('.campus-picker').hidden = state.campusConfig.length < 2;
  if (state.selectedCampus) ui.campusSelect.value = state.selectedCampus.id;
  renderHeaderContext();
}
...
function renderHeaderContext() {
  ...
  const badgeClass = ['gps', 'manual'].includes(state.detection.quality) ? 'badge-gps' : state.detection.quality === 'approx' ? 'badge-approx' : 'badge-unknown';
  ui.campusBadge.className = `badge ${badgeClass}`;
  ui.campusBadge.textContent = state.detection.quality === 'gps' ? 'GPS' : state.detection.quality === 'manual' ? 'Choisi' : state.detection.quality === 'approx' ? 'Approx.' : 'Inconnu';
...
function computeRoomSummaries() {
  ...
  for (const ev of state.cache.events || []) {
    const locations = splitLocations(ev.location).filter(isPhysicalRoom);
    ...
    for (const loc of locations) {
      ...
      map.get(loc).push({ ...ev, location: loc, start, end });
    }
  }
...
function isPhysicalRoom(location) {
  return !/^(?:a distance|en direct à distance)$/i.test(location);
}
